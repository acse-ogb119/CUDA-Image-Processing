...
__global__ void naive_normalized_cross_correlation(float *d_response, unsigned char *d_original, unsigned char *d_template,
                                                   int num_pixels_y, int num_pixels_x, int template_half_height, int template_height,
                                                   int template_half_width, int template_width, int template_size, float template_mean)
{
    int ny = num_pixels_y;
    int nx = num_pixels_x;
...
{
    __host__ __device__
        thrust::tuple<unsigned char, unsigned char, unsigned char>
...
void preProcess(unsigned int **inputVals, unsigned int **inputPos,
                ...
                size_t &rows, size_t &cols, size_t &templateHalfHeight, size_t &templateHalfWidth,
                ...
                const std::string &templateFilename)
{
    ...
    loadImageRGBA(filename, &inImg, &rows, &cols);
    ...
    thrust::device_vector<uchar4> d_Img(inImg, inImg + rows * cols);
    ...
    //split the image
    thrust::transform(d_Img.begin(), d_Img.end(), thrust::make_zip_iterator(thrust::make_tuple(d_red.begin(), d_blue.begin(), d_green.begin())),
                      splitChannels());
    //split the template
...
__global__ void remove_redness_from_coordinates(const unsigned int *d_coordinates, unsigned char *d_r_output,
                                                unsigned char *d_r, unsigned char *d_b, unsigned char *d_g,
                                                int num_coordinates, int num_pixels_y, int num_pixels_x,
                 ...
                 const std::string &output_file)
{
    ...
    saveImageRGBA(&h_Img[0], rows, cols, output_file);
}

void thrust_sort(unsigned int *const d_inputVals,
                 ...
                 const int rows, const int cols)
{
    ...
    thrust::device_ptr<unsigned int> dev_ptr_outputVals(d_outputVals);
    thrust::device_ptr<unsigned int> dev_ptr_outputPos(d_outputPos);

    thrust::sort_by_key(thrust::device, dev_ptr_outputVals, dev_ptr_outputVals + numElems, dev_ptr_outputPos, thrust::less<unsigned int>());
}

void remove_redeye(const std::string &input_file, const std::string &template_file, const std::string &output_file)
{
    size_t numRows, numCols;
    size_t templateHalfWidth, templateHalfHeight;

    unsigned int *inputVals, *inputPos;
    unsigned int *outputVals, *outputPos;
    thrust::device_vector<unsigned char> d_red;
    thrust::device_vector<unsigned char> d_blue;
    thrust::device_vector<unsigned char> d_green;

    preProcess(&inputVals, &inputPos, &outputVals, &outputPos, d_red, d_blue, d_green,
               numRows, numCols, templateHalfHeight, templateHalfWidth,
               input_file, template_file);

    thrust_sort(inputVals, inputPos, outputVals, outputPos, numRows, numCols);

    postProcess(outputVals, outputPos, d_red, d_blue, d_green,
                numRows, numCols, output_file);
